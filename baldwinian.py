...
EXP_ID = 'baldwinism_version'  # the ID of this experiment (used to create log names)
...
class baldwinian_evol:
    ...
    def selection(self, pop, fits_dead, mate_sel):
        mating_pool = mate_sel(pop, [f_born - f_dead for f_born, f_dead in zip(self.fits_born, fits_dead)],POP_SIZE)
        self.fits_born = fits_dead
        return (mating_pool)


def evol_algo(pop, max_gen, fitness, operators, mate_sel, mutate_ind, *, map_fn=map, log=None):
    evolution = baldwinian_evol(fit_func=fitness)
    evals = 0
