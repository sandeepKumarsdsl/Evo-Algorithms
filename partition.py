...
def bin_weights_mutate(weights, bins):
    bw = [0]*K
    for w, b in zip(weights, bins):
        bw[w] += b
    return bw
...
def informed_mutate(ind, ws):
   
    bw = bin_weights(ws, ind)
    highest = bw.index(max(bw))
    lowest = bw.index(min(bw))
    k = 0
...
def mutation(pop, mutate, mut_prob):
    ...
    if random.random() < mut_prob:
        j = random.randrange(1,len(i))
        k = random.randrange(1,len(i))
        if j != k:
            i[j], i[k] = i[k], i[j]
    return i
    """
...
if __name__ == '__main__':
    ...
    #mutate_ind = informed_mutate(pop, ws=weights)
    mut = functools.partial(mutation, mut_prob=MUT_PROB,mutate=functools.partial(informed_mutate, weights))
