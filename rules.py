...
INPUT_FILE = 'winequality-white.csv' # the input file for classification
OUT_DIR = 'rules' # output directory for logs
EXP_ID = 'winequality-white_weighted' # the ID of this experiment (used to create log names)
...
def weight_mutate(p, mut_weight_prob_change, mut_weight_sigma):
    p = copy.deepcopy(p)
    o = []
    for r in p:
        o_cls=r.cls
        if random.random() < mut_weight_prob_change:
            new_weight = r.weight + mut_weight_sigma*np.random.randn()
        else:
            new_weight = r.weight
        o.append(Rule(conditions=r.conditions, cls=o_cls, weight=new_weight))
    return o
...
if __name__ == '__main__':
    ...
    #mut_weight_prob_chage=MUT_WEIGHT_PROB_CHANGE, mut_weight_sigma=MUT_WEIGHT_SIGMA
    #cls_mutate = functools.partial(cls_mutate, num_classes=num_classes)
    w_mutate = functools.partial(weight_mutate, mut_weight_prob_chage=MUT_WEIGHT_PROB_CHANGE, mut_weight_sigma=MUT_WEIGHT_SIGMA)
    mut_cls = functools.partial(mutation, mutate=w_mutate, mut_prob=MUT_CLS_PROB)
    mut_cond = functools.partial(mutation, mutate=cond_mutate, mut_prob=MUT_COND_PROB)
    #mut_cond2 = functools.partial(mutation, mutate=weight_mutate, mut_prob=MUT_COND_PROB)
    ...
    utils.plot_experiment(evals, lower, mean, upper, legend_name = 'Default_weighted')
    plt.legend()
    plt.yscale('log')
    plt.show()