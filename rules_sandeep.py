...
MUT_COND_PROB = 0.5 # probabilty of condition changing mutation
MUT_COND_SIGMA = 0.1 # step size of condition changing mutation
REPEATS = 10 # number of runs of algorithm (should be at least 10)
INPUT_FILE = 'iris.csv' # the input file for classification
OUT_DIR = 'rules' # output directory for logs
EXP_ID = 'rules_changed' # the ID of this experiment (used to create log names)
...
class between:
    ...
    def __init__(self, threshold1,threshold2, lb, ub):
        ...
        self.params = np.array([threshold1, threshold2])
        self.lb = lb
        self.ub = ub
    ...
    def __call__(self, value):
        if self.boundary2() > self.boundary1():
            return (value <= self.boundary2() and value >= self.boundary1())
        else:
            return (value <= self.boundary1() and value >= self.boundary2())
    def __str__(self):
         return ">= " + str(self.boundary1()) +"and <= " + str(self.boundary2())
...
class notbetween:
    ...
    def __call__(self, value):
        if self.boundary2() > self.boundary1():
            return (not(value <= self.boundary2()) and not(value >= self.boundary1()))
        else:# self.boundary2() <= self.boundary1():
            return (not(value <= self.boundary1()) and not(value >= self.boundary2()))
    ...
    for r1,r2 in zip(o1,o2):
        for c1,c2 in zip(r1.conditions,r2.conditions):
            if random.random()>0.8:
                ...
                c1.params += MUT_COND_SIGMA*np.random.randn(*c1.params.shape)
...
if __name__ == '__main__':
    ...
    #mut_cond2= functools.partial(mutation, mutate=cond_mutate2, mut_prob=MUT_COND_PROB)
    ...
    utils.plot_experiment(evals, lower, mean, upper, legend_name = 'rules_mutation_changes')
    plt.legend()
    plt.yscale('log')
    plt.show()